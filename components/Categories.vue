<template>
  ...
  >
    ...
    <div v-if="isLoading">Loading...</div>
      ...
      <div
        class="sticky top-0 z-10 flex flex-no-wrap max-w-full pl-1 mb-2 overflow-x-auto text-gray-700 bg-gray-100 border-t shadow-lg"
      ...
      >
        <nuxt-link
          ...
          :key="c.id"
          :to="`/c/${c.slug}`"
          class="w-full mx-auto category-item"
        >
          <img v-lazy="c.img" class="w-10 h-10 mb-2 rounded-full" alt />
          <span class="w-full mx-auto text-xs text-center">{{ c.name }}</span>
        </nuxt-link>
      </div>
    </template>
  </ApolloQuery>
