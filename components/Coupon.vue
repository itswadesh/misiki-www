<template>
  <div
    ...
    :class="color"
  >
    <div class="flex items-center mt-2 mb-2 ml-8">
      <div class="w-2/3">
        <h2 class="text-4xl font-bold">{{ code }}</h2>
        <p class="mb-4 -mt-2 leading-tight text-gray-400">{{ text }}</p>
      </div>
      <div class="w-1/3 mx-4 text-center">
        <button
          @click="applyOffer(code)"
          class="px-6 text-xl text-gray-400 rounded-lg hover:text-white"
    ...
    <div class="py-1 text-xs text-center bg-black opacity-25">
      <p class="text-white opacity-100">{{ terms }}</p>
    </div>
  </div>
</template>
